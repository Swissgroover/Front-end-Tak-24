# Front-end kursuse punktisüsteem: “Koodi ja Käitumise Tasakaal”

Kõik õpilased alustavad **10 punktiga**.  
Punktid peegeldavad **sotsiaalset ja professionaalset vastutust** — sama tähtsat kui tehnilised oskused.

---

## Alustase
Kõik õpilased alustavad **10 punktiga**

---

## 🔻 Punktide kaotus – professionaalse käitumise rikkumised

| Käitumine või olukord | Punktide kadu | Selgitus |
|-----------------------|---------------:|----------|
| Hilinemine tundi | -1 | Ebausaldusväärne |
| Puudumine  | -2 | Ebausaldusväärne |
| Kodutöö mitte esitamine tähtaegselt | -2 | Professionaalse rikkumine |
| Tunni segamine või lugupidamatu käitumine | -1 | Häirib tööõhkkonda |
| Teiste töö pealt maha tegemine | -5 | Ebasoovitav meeskonnakäitumine |
| Spikerdamine / koodi kopeerimine ilma mõistmiseta | -3 | Eetika ja õpioskuse rikkumine |
| Ropp või lugupidamatu suhtlus (sh online-kanalites) | -2 | Meeskonna turvalisuse rikkumine |
| Tunniülesande mitte täitmine ettenatud kuupäevaks/kellaajaks | -5 | Kehtib ka tunnis antud töödele |


---

## 🌱 Punktide teenimine tagasi

| Käitumine | Boonuspunktid | Selgitus |
|-----------|---------------:|----------|
| Kaasõpilase abistamine tehnilises küsimuses | +1 | Meeskonna tugi ja teadmiste jagamine |
| Väike parendus või bugifix projekti heaks vabatahtlikult | +1 | Initsiatiiv ja vastutustunne |
| Hea meeskonnatöö ja kommunikatsioon projektis | +1 | Professionaalne hoiak |

---

## 🎯 Punktide mõju lõputööle (30 akadeemilise tunni järel)

| Punktivahemik | Lõputöö keerukus | Selgitus / töö tüüp |
|---------------|-------------------|-----------------------|
| **9 punkti** | Lihtsam API-projekt | Näiteks: **Avaliku API põhine kuvamisleht** – Pokémoni või NASA pildiarhiivi API kasutamine. Andmed kuvatakse Tailwindiga. Keskendutakse koodi puhtusele ja struktuurile. (Vue/React) |
| **7–8 punkti** | Lisaks Interaktiivne andmete kuvamine | Näiteks: **Valuuta- või ilmaandmete rakendus**, kus kasutaja saab otsida ja filtreerida API vastuseid. Vajalik olekuhaldus (state) ja komponentide vahel andmete liikumine. |
| **5–6 punkti** | Mölemad eelnevad rakendused | Õpilane koostab mölemad eelnevad etapid sammasse rakendusse. Multi page application |
| **3–4 punkti** | Mitme API ühendus + dokumentatsioon + Figma Disain | Peavad olema tehtud ka **9–10**, **7–8** ja **5–6** taseme tööd ühel lehel. Vajalik andmete kombineerimine ja dünaamiline renderdus. Projekti juurde kuulub põhjalik arendusdokumentatsioon, mis kirjeldab tehnilist lahendust. |
| **0–2 punkti** | Täielik projekt + täisanalüüs + kõik eelnevad tasemed | Peavad olema tehtud ka **9–10**, **7–8**, **5–6** ja **3–4** taseme tööd ühel lehel. Vajalik andmete töötlemine, sorteerimine, graafikud (Chart.js / Recharts) ja põhjalik README koos refleksiooniga: „Kuidas minu käitumine mõjutas projekti ja meeskonda?”. |


## 📊 Õpilaste punktitabel

| Õpilase nimi | Algpunktid | Kaotatud punktid | Teenitud lisapunktid | Hetkeseis kokku | Märkused |
|---------------|-------------:|-----------------:|---------------------:|----------------:|-----------|
|Marius         | 10 |  6 |   |  4 |   |
|Joosep         | 10 |  3 |   | 7  |   |
|Carlos         | 10 |  4 |   |  6 |   |
|Eli            | 10 |  4 |   |  6 |   |
|Rasmus         | 10 |  16 |   | -6  |   |
|Gregor         | 10 |  4 |   |  6 |   |
|Keiro          | 10 |  2 |   |  8 |   |
|Silvio         | 10 |  1 |   |  9 |   |
|Jasper         | 10 |  2 |  1 |  9 |   |
|Matis          | 10 |  3 |   | 7 |   |
|Oskar          | 10 |  8 |  2 | 4  |   |
|Ragnar         | 10 |   |  1 |  11 |   |



